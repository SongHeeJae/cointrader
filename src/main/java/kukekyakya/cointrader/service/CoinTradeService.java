...
import kukekyakya.cointrader.client.UpbitClientService;
...
import kukekyakya.cointrader.service.wallet.BuyCoin;
...
public class CoinTradeService {
	...
	private static final int MINUTE_UNIT = 15; // 최근 매매했던건 이 시간동안은 구매 안하는 것도 필요할듯.
	private static final int COUNT = 1;
	private static final double DIFF_PERCENT = 0.02; // 봉의 시가, 현재 종가 기준으로 이 정도 이상 차이나는 경우
	private static final double SELL_PERCENT_P = 0.01; // 양일 떄 판매 퍼센트 설정
	private static final double SELL_PERCENT_M = 0.015; // 음일 떄 판매 퍼센트 설정
	private static final long MINUS_CANDLE_COUNT = 12; // COUNT 보다 작아야한다.

	private static final int START_BUY_MONEY = 10000;
	private static final double MINIMUM_PRICE = 50; // 구매하고자 하는 코인의 최소 개당 금액
			...
			for (Market market : markets) {
				...
				if (percent >= DIFF_PERCENT) { // TODO 거래량도 확인
					...
					if(lockDateTime != null) {
						if(lockDateTime.isAfter(LocalDateTime.now())) {
							BUY_LOCK_DATE_TIME.remove(market.getMarket());
							lockDateTime = null;
						}
					}

					if(lockDateTime != null) continue;
